...
try{
    ...
    const response = await axiosInstance.post(`${api}/v1/rent/user/newAccommodation`, accomodationData);
    if(!response){
...
export const getAllAccomodation =() => async(dispatch)=>{
    try{
       dispatch(accomodationActions.getAccomodationRequest());
       const{data} = await axiosInstance.get(`${api}/v1/rent/user/myAccommodation`);
